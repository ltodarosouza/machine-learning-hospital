...
| 2026-08-13 | Issue #79 | A simulação de impacto aceita fator de segurança único ou mapa completo por medicamento; políticas reutilizáveis agrupam perfil de demanda e faixa de prazo, sem configurar produto a produto. `estoque_medio_unidades` passou a explicitar o trade-off de estoque na avaliação. A política candidata não altera ainda o motor/dashboard, pendente de validação formal com janelas longas (#84). | recommendation, evaluation |
